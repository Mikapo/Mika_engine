...
//  2021-12-15: OpenGL: Using buffer orphaning + glBufferSubData(), seems to fix leaks with multi-viewports with some
//  Intel HD drivers. 2021-08-23: OpenGL: Fixed ES 3.0 shader ("#version 300 es") use normal precision floats to avoid
//  wobbly rendering at HD resolutions. 2021-08-19: OpenGL: Embed and use our own minimal GL loader
//  (imgui_impl_opengl3_loader.h), removing requirement and support for third-party loader. 2021-06-29: Reorganized
//  backend to pull data from a single structure to facilitate usage with multiple-contexts (all g_XXXX access changed
//  to bd->XXXX). 2021-06-25: OpenGL: Use OES_vertex_array extension on Emscripten + backup/restore current state.
//  2021-06-21: OpenGL: Destroy individual vertex/fragment shader objects right after they are linked into the main
//  shader. 2021-05-24: OpenGL: Access GL_CLIP_ORIGIN when "GL_ARB_clip_control" extension is detected, inside of just
//  OpenGL 4.5 version. 2021-05-19: OpenGL: Replaced direct access to ImDrawCmd::TextureId with a call to
    ...
    ImDrawData* draw_data, int fb_width, int fb_height, GLuint vertex_array_object)
{
    ...
    glVertexAttribPointer(
        bd->AttribLocationVtxPos, 2, GL_FLOAT, GL_FALSE, sizeof(ImDrawVert), (GLvoid*)IM_OFFSETOF(ImDrawVert, pos));
    glVertexAttribPointer(
        bd->AttribLocationVtxUV, 2, GL_FLOAT, GL_FALSE, sizeof(ImDrawVert), (GLvoid*)IM_OFFSETOF(ImDrawVert, uv));
    glVertexAttribPointer(
        bd->AttribLocationVtxColor, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(ImDrawVert),
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                glScissor(
                    (int)clip_min.x, (int)((float)fb_height - clip_max.y), (int)(clip_max.x - clip_min.x),
...
static bool CheckProgram(GLuint handle, const char* desc)
{
    ...
    if ((GLboolean)status == GL_FALSE)
        fprintf(
            stderr, "ERROR: ImGui_ImplOpenGL3_CreateDeviceObjects: failed to link %s! With GLSL %s\n", desc,
            bd->GlslVersionString);
    if (log_length > 1)
    {
        ImVector<char> buf;
